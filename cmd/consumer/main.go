...
import (
	...
	"github.com/raffreitas/go-kafka/internal/infra/database"
	"github.com/raffreitas/go-kafka/internal/usecase"
	"github.com/raffreitas/go-kafka/pkg/kafka"
...
func main() {
	...
	defer db.Close() // executa tudo e depois fecha a conexão
	...
	severs := "host.docker.internal:9094"

	fmt.Println("Kafka consumer has started")
	go kafka.Consume(topics, severs, msgChanKafka)
...
func kafkaWorker(msgChan chan *ckafka.Message, uc usecase.CalculateFinalPrice) {
	...
	for msg := range msgChan {
		...
		fmt.Printf("Kafka has processed %s\n", outputDto.ID)
	}
}
